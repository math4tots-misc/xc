...
struct SharedPtr {
  ...
  xct_Bool operator==(SharedPtr<T> p) const {
    return ptr == p.ptr || ptr->xcm_operator_equals(p);
  }
  ...
  xct_Bool operator<(SharedPtr<T> p) const {
    return ptr->xcm_operator_less_than(p);
  }
...
struct xcs_String: xcs_Object {
  ...
  xct_Bool xcm_operator_less_than(xct_String s) {
    return data < s->data;
  }
};
...
template <class T> struct xcs_List: xcs_Object {
  ...
  xct_Bool xcm_operator_equals(xct_List<T> p) const {
    return data == p->data;
  }

  template <class F>
  auto xcm_map(F f) -> xct_List<decltype(f(T()))> {
  ...
  xct_Bool xcm_operator_less_than(xct_List<T> s) {
    return data < s->data;
  }
};
...
template <class K, class V> struct xcs_Map: xcs_Object {
  ...
  xct_Void xcm_set(K key, V value) {
    data[key] = value;
  }
...
// xct_Int xcv_int(xct_String s, xct_Int base) {
...
class Translator(object):
  ...
  def parse_primary_expression(self):
    ...
    if self.consume('('):
      ...
      if self.trace:
        body = patch_block_with_stack_frame(
            body, self.source.filespec, '<anonymous>',
            token.lineno())
